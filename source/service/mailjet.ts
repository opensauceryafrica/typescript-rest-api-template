...
const transporter = new Resend(env.mail.resend.apiKey);
...
export const sendMail = async (
    ...
    data?: object,
    attachments?: Array<string>,
): Promise<CreateEmailResponseSuccess | string | Error> => {
    return new Promise((resolve, reject) => {
        transporter.emails
            .send({
                to: to,
                from: `${env.mail.name} <${env.mail.from}>`,
                subject: subject,
                html: func.renderTemplateString(template, data || {}),
                attachments: func.extractAttachments(attachments || []),
            })
            .then((status: CreateEmailResponse) => {
                if (status.error) {
                    logger.debug(
                        `[${new Date().toISOString()}] :: error while sending mail to ${to} :: ${
